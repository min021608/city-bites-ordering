...
這是一個簡易的線上訂餐網站，提供餐點瀏覽、分類與搜尋、購物車、外送或自取選擇、結帳送出訂單，以及集中式訂單統計。
...
- 將餐點加入購物車、調整數量。
- 自動計算外送費和滿額折扣：外送滿 `NT$399` 免運、滿 `NT$500` 折 `NT$50`。
- 選擇外送或自取，填寫結帳資料並顯示訂單成立訊息。
- 使用「編輯菜單」自行修改餐點，並由後端保存菜單。
- 送出訂單後由後端集中保存，以「訂單統計」查看今日訂單、營業額、熱銷品項與訂單明細。
